...
def cal_stat(Total_STAT_ARRAY, Total_IGNORE):
    ...
    NET_DEX = math.floor(a * (1 + T[3,0]/100) + T[13,0]) # 메용O
    NET_DEX2 = math.floor((base_stat + T[2,0] + T[5,0] * lv_ratio) * (1 + T[3,0]/100) + T[13,0]) # 메용X
...
def parse_character_stats(Total_STAT_ARRAY, NET_STAT):
    ...
    return {
        ...
        'Critical_Damage': int(T[11]) + int(T[33]) + 5, # 기본+노블+도핑
        'DEX_with_MY': N[2],
        'STR': N[3],
