...
def PF_cal_damage_burst2(plot_graph, ring, VI_level, time, print_property, directory):
    start_time = tm.time()
    Boss_Guard = 380
    stats = get_final_stats(directory, print_property, 0)
    ascent_stats = get_final_stats(directory, print_property, 1)
    Condition_ARRAY1, Condition_ARRAY2 = burst_time2_3()
    ...
    def D(pct, base_ign, add_boss=0, add_ign=0, f_dmg=1.0):
        base_dmg = calc_D(pct, base_ign, Boss_Guard, stats, Damage2, Boss_Damage2, Critical_Damage2, DEX2, Attack_Ratio2, add_boss, add_ign, f_dmg)
        return base_dmg * (FINAL_ATTACK / stats["FINAL_ATTACK"])
    ...
    Discharge_Used = Res_stack = Material_stack = Material_attack = Additional_forbidden = 0
    Additional_Blast_Curse_Arrow = Forsaken_Arrow = Ancient_Fury = 0
    Material_Burst_Count = Material_Burst_Remaining = 0
    Active = 1
    ...
    Burst_Active_Flag = np.zeros(time)
        ...
        if VI_level[9] > 0:
            ...
            if 0 <= elapsed_pen <= 6600:
                ...
                if elapsed_pen in pen_schedule:
                    ...
                    attacks = 15 * times
                    pct_penetration = (883 + 174 * VI_level[9]) * attacks
                    lvl = VI_level[9]
 ...
 #-----------버프류 설정 구간--------------------------------------------------------------------------------------------------------------------          
        ...
        if stats["Just_One"] == 1 and 45000 <= (t % T120) < 50000:
            Active = 0
        ...
        if stats["Fatal_Strike"] == 1 and Condition_ARRAY[12] <= t % (30*1000) < Condition_ARRAY[12] + 4*1000:
            Boss_Damage2 += 100
        if stats["Just_One"] == 1 and Condition_ARRAY[12] <= t % T120 < Condition_ARRAY[12] + 10*1000:
            Critical_Prob2 += 100
        if stats["Just_One"] == 2 and Condition_ARRAY[12] <= t % (70*1000) < Condition_ARRAY[12] + 10*1000:
            Critical_Prob2 += 100
...
#-------------------------------------------------------------------------------------------------------------------------------            
        ...
        if 0 <= elapsed_ultimate <= 1350:
            극딜 = 1
        if elapsed_ultimate == 1370:
            Ultimate_Blast = D(VI_level_factor[3] * 6 * 3000 * 15, 100, Ancient_Enchant_Boss_Damage, 0, Ancient_Enchant_Final_Damage)           
        ...
        Material = 0
        cycle_time = t % T120
        Material_stack += Material_attack
        Material_attack = 0
        if Material_stack > 0 and VI_level[8] > 0:
            Material = D(Material_stack * 2 * (672 + 11 * VI_level[8]), Ignore_Guard2)               
            state["Material"]["times_used"] += Material_stack
            state["Material"]["attacks"] += 2 * Material_stack
            Material_stack = 0
        ...
        if VI_level[10] > 0:
            ...
            if 0 <= elapsed_styx <= 3660:
                styx_bonus = 20 * min(2, VI_level[10] // 10) 
                attacks = 0
                if elapsed_styx == 0:
                    attacks, pct = 8, (100 + 4 * VI_level[10])
                if attacks > 0:
        ...
        if cycle_time >= Condition_ARRAY[17] and 극딜 == 0:
            ...
            time_to_next_burst = T120 - cycle_time + Condition_ARRAY[17]
            can_use_assault = time_to_next_burst >= cd_ms
            if VI_level[8] > 0:
                if state["Assault"]["delay"] <= 0 and Active == 1 and can_use_assault:
                    ...
                    Material_attack += 4 * 0.6 * Resonance_Num
                    ...
                    if VI_level[7] > 0:
                        ...
                        state["Resonance"]["times_used"] +=  3 * Resonance_Num
                        state["Resonance"]["attacks"] += 18 * Resonance_Num
                    Material_attack += 3 *0.6 * Resonance_Num
                    #콤레 이후 블래스트부터 발동
        ...
        # Material Burst, Ancient Burst
        Material_Burst = 0
        Ancient_Burst = 0
        if t % T60 == Condition_ARRAY[18] and VI_level[12] > 0:
            Material_Burst_Remaining = 6
        if Material_Burst_Remaining > 0:
            Burst_Active_Flag[t // 1000] = 1
            if state["Material_Burst"]["delay"] <= 0:
                Material_Burst_Count += 1
                if Material_Burst_Count % 3 == 0:
                    pct_ancient = 12 * (696 + 42 * VI_level[12]) + 12 * 4 * (800 + 49 * VI_level[12])
                    Ancient_Burst = D(pct_ancient, Ignore_Guard2, Ancient_Enchant_Boss_Damage, Ancient_Enchant_Ignore_Guard, Ancient_Enchant_Final_Damage)
                    state["Ancient_Burst"]["times_used"] += 5
                    state["Ancient_Burst"]["attacks"] += 60
                    Material_attack += 5
                else:
                    pct_material = 6 * 4 * (229 + 13 * VI_level[12])
                    Material_Burst = D(pct_material, Ignore_Guard2, Ancient_Enchant_Boss_Damage, Ancient_Enchant_Ignore_Guard, Ancient_Enchant_Final_Damage)
                    state["Material_Burst"]["times_used"] += 4
                    state["Material_Burst"]["attacks"] += 24
                    Material_attack += 4
                Material_Burst_Remaining -= 1
                state["Material_Burst"]["delay"] = 300 - iteration_time
            else:
                state["Material_Burst"]["delay"] -= iteration_time
        else:
            state["Material_Burst"]["delay"] -= iteration_time
        ...
        if 극딜 == 0 and cycle_time >= Condition_ARRAY[10]: #얼블 캔슬 삭제
            # Cardinal Blast
        ...
        if Forsaken_buff == 1:
            ...
            # Ancient Fury
            if Condition_ARRAY[16] <= t % T360 <= Condition_ARRAY[16] + (30+stats["Server_Lag"])*1000:
                if state["Ancient_Fury"]["delay"] <= 0 and (Cardinal_Blast or Cardinal_Discharge or Assault or Forsaken_Relic) > 0:
        ...
        if VI_level[10] > 0 and hecate_lock == 0:
            if state["Hecate"]["delay"] <= 0:
                ...
                Hecate = D((370 + 27 * VI_level[10]) * 24, Ignore_Guard2, h_bonus, h_bonus)
        ...
        skill_values = [
            ...
            Hecate, Styx, Phlegethon, Penetration, Evolve_Orbit, 
            Material_Burst, Ancient_Burst, test_graph
        ]
    ...
    return total_sum, Deal_Int_C, Deal_Int_CC, time_int #, Burst_Active_Flag
