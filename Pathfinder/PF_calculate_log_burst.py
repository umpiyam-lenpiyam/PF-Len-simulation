...
def PF_cal_damage_CR2_burst(plot_graph, ring, VI_level, time, print_property, directory):
    start_time = tm.time()
    Boss_Guard = 380
    stats = get_final_stats(directory, print_property, 0)
    ascent_stats = get_final_stats(directory, print_property, 1)
    Condition_ARRAY1, Condition_ARRAY2 = burst_time2()
    ...
    Material_Burst_Stack = Material_Burst_Active = Material_Burst_Count = 0
    Active = 1
        ...
        if VI_level[9] > 0:
            ...
            if 0 <= elapsed_pen <= 6600:
                ...
                if elapsed_pen in pen_schedule:
                    ...
                    attacks = 15 * times
                    pct_penetration = (903 + 176 * VI_level[9]) * attacks
                    lvl = VI_level[9]
 ...
 #-----------버프류 설정 구간--------------------------------------------------------------------------------------------------------------------          
        ...
        if t % T120 == Condition_ARRAY[13] and VI_level[12] > 0:
            Material_Burst_Stack += 10
            Material_Burst_Active += Material_Burst_Stack
            Material_Burst_Stack = 0
        if t % 3500 == 0:
            Material_Burst_Stack += 1
        if t % T120 == int(Condition_ARRAY[13] + 30*1000 + svr_lag) and VI_level[12] > 0:
            if Material_Burst_Active > 6:
                Material_Burst_Active = 6
...
#-------------------------------------------------------------------------------------------------------------------------------            
        ...
        Material_Burst = 0
        Ancient_Burst = 0
        if 극딜 == 0 and Material_Burst_Active > 0 and Active == 1 and Assault_between_delay < 0 and Forsaken_buff == 0:
            if state["Material_Burst"]["delay"] <= 0:
                Material_Burst_Count += 1
                if Material_Burst_Count % 3 == 0:
                    pct_ancient = 12 * (670 + 8 * VI_level[12]) + 12 * 4 * (786 + 9 * VI_level[12]) 
                    ...
                    pct_material = 6 * 4 * (228 + 2 * VI_level[12])
        ...
        if 극딜 == 0 and cycle_time >= Condition_ARRAY[17]:
            # Cardinal Blast
            if state["Cardinal_Blast"]["delay"] <= 0 and Active == 1 and Assault_between_delay <= 0 and (Material_Burst_Active <= 0 or Forsaken_buff == 1):
                if VI_level[5] > 0:
            ...
            # Cardinal Discharge
            if state["Cardinal_Discharge"]["delay"] <= 0 and Active == 1 and Cardinal_between_Delay <= 0 and Assault_between_delay <= 0 and (Material_Burst_Active <= 0 or Forsaken_buff == 1):
                if VI_level[6] > 0:
        ...
        if Forsaken_buff == 1:
            ...
            # Ancient Fury
            if Condition_ARRAY[16] <= t % T360 <= Condition_ARRAY[16] + (30+stats["Server_Lag"])*1000:
                if state["Ancient_Fury"]["delay"] <= 0 and (Cardinal_Blast or Cardinal_Discharge or Assault) > 0:
    ...
    idx_material = skill_order.index("Material_Burst")
    idx_ancient = skill_order.index("Ancient_Burst")
    material_dmg = Deal_Cumulative[idx_material]
    ancient_dmg = Deal_Cumulative[idx_ancient]
    print(f"[추출] 신규 스킬 합산 데미지: {(material_dmg + ancient_dmg):.4f} 조")
