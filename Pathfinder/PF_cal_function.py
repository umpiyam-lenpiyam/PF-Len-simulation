...
def burst_time2_2(): # 딸깍 빌드
    delays = {'Sequence_Start': 60, 'Sequence_End': 60-90,
        '배리어': 90, '에픽어드벤쳐': 90, '메용2': 90, '크리인': 90, '엔버': 90, '시드링': 90, '이볼브': 90,
        '템페스트': 540, '포세이큰': 6540, '얼블': 1350, '렐릭에볼루션': 90, '언바': 90, # 300, 540
        '평딜시작': 0, '콤레': 510, '버스트': 1800}
    
    idx = {'에픽어드벤쳐': 0, '메용2': 1, '크리인': 2, '엔버': 3, '시드링': 4, '템페스트': 5, '언바': 6, '배리어': 7, '얼블': 8,
        '렐릭에볼루션': 13, '이볼브': 14, '포세이큰': 16, '평딜시작': 10, 'Boss_Slayer': 11, 'Fatal_Strike': 12, '콤레': 17, '버스트': 18}

    # 6차 극딜   
    sequence1 = ['Sequence_Start', '에픽어드벤쳐', '메용2', '크리인', '이볼브', '엔버', '시드링', '배리어', '언바', '렐릭에볼루션', 
        'Sequence_End', '콤레', '템페스트', '포세이큰', '버스트', '얼블', '평딜시작']
    
    # 5차 극딜
    sequence2 = ['Sequence_Start', '에픽어드벤쳐', '메용2', '크리인', '이볼브', '엔버', '시드링', '배리어', '언바', '렐릭에볼루션', 
        'Sequence_End', '콤레', '템페스트', '버스트', '얼블', '평딜시작']
...
def burst_time2_3(): # 쪼금 신경 쓴 빌드
    delays = {'Sequence_Start': 60, 'Sequence_End': 60-90,
        '배리어': 90, '에픽어드벤쳐': 90, '메용2': 90, '크리인': 90, '엔버': 90, '시드링': 90, '이볼브': 90,
        '템페스트': 540, '포세이큰': 6540, '얼블': 1350, '렐릭에볼루션': 120, '언바': 120, # 300, 540
        '평딜시작': 0, '콤레': 510, '버스트': 1800}
    ...
    # 6차 극딜
    sequence1 = ['Sequence_Start', '에픽어드벤쳐', '메용2', '크리인', '이볼브', '엔버', '시드링', '배리어',
        'Sequence_End', '콤레', '템페스트', '포세이큰', '언바', '렐릭에볼루션', '버스트', '얼블', '평딜시작']
    
    # 5차 극딜
    sequence2 = ['Sequence_Start', '에픽어드벤쳐', '메용2', '크리인', '이볼브', '엔버', '시드링', '배리어', 
        'Sequence_End', '콤레', '템페스트', '언바', '렐릭에볼루션', '버스트', '얼블', '평딜시작']
...
def write_skill_log(total_sum, Deal_Cumulative, deal_order, state, filename='PF_log.txt'):
    ...
    log_targets = [
        ...
        ('마테리얼 버스트', ["Material_Burst"]),
        ('에인션트 버스트', ["Ancient_Burst"])
    ]
    ...
    if plot_graph in [2, 3]:
        ...
        if plot_graph == 2:
            ...
            colors = [
                ...
                'crimson',        # 23: 플레게톤
                'silver',         # 24: 렐릭 페네트레이션
                'lightskyblue',   # 25: 칠흑의 궤적
                'mediumblue',         # 26: 마테리얼 버스트
                'mediumblue'          # 27: 에인션트 버스트
            ]
...
def compare_graph(plot_graph, time_int1, Deal_Int_C1, Deal_Int_C2, Deal_Int_CC1, Deal_Int_CC2, total_sum1, total_sum2):
    ...
    if plot_graph == 4:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            Deal_Int_CC3 = np.where(Deal_Int_CC1 != 0, (Deal_Int_CC2 - Deal_Int_CC1) / Deal_Int_CC1, 0)
        plt.style.use('dark_background')
