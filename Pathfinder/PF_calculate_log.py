...
def PF_cal_damage(plot_graph, ring, VI_level, time, print_property, directory):
    start_time = tm.time()
    Boss_Guard = 380
...
#-------------------------------------------------------------------------------------------------------------------------------            
        ...
        if cycle_time >= Condition_ARRAY[10]:
            ...
            else: # 콤썰트 미개방 시 -> 레조만, (디스차지 이후에 발동)
                if state["Resonance"]["delay"] <= 0 and Discharge_Used == 1 and Active == 1:
                    if VI_level[7] > 0:
                        if Res_stack < 2:
                            pct_res = res_mult * 3 * (943 + 33 * VI_level[7]) * 6 * 2.2 * (1.1**5)
                            Resonance = D(pct_res, VMatrix_Ignore_Guard, Ancient_Enchant_Boss_Damage, Ancient_Enchant_Ignore_Guard, Ancient_Enchant_Final_Damage)
                            Res_stack += 1
                            state["Resonance"]["attacks"] += 18 * Resonance_Num
                            state["Resonance"]["times_used"] +=  3 * Resonance_Num
                        else:
                            pct_res = res_mult * 3 * (616 + 35 * VI_level[7]) * 10 * 2.2 * (1.1**5)
                            Resonance2 = D(pct_res, VMatrix_Ignore_Guard, Ancient_Enchant_Boss_Damage, Ancient_Enchant_Ignore_Guard, Ancient_Enchant_Final_Damage)
                            Res_stack = 0
                            state["Resonance2"]["attacks"] += 30 * Resonance_Num
                            state["Resonance2"]["times_used"] += 3 * Resonance_Num
                    else:
                        pct_res = res_mult * 898 * 3 * 6 * 2.2 * (1.1**5)
                        Resonance = D(pct_res, VMatrix_Ignore_Guard, Ancient_Enchant_Boss_Damage, Ancient_Enchant_Ignore_Guard, Ancient_Enchant_Final_Damage)
                        state["Resonance"]["times_used"] +=  3 * Resonance_Num
                        state["Resonance"]["attacks"] += 18 * Resonance_Num   
                    state["Resonance"]["delay"] = cd_ms - iteration_time
                    Material_stack += 1.5 * Resonance_Num
                    Cardinal_between_Delay = 480 # 이거 왜 썼지? 몰루
                else:
                    state["Resonance"]["delay"] -= iteration_time
            ...
            if VI_level[5] > 0:
                if state["Additional_Blast_Curse_Arrow"]["delay"] <= 0:
