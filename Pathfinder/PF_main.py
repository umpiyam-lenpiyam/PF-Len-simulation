...
from PF_cal_burst import *
from PF_cal_burst2 import *
...
VI_level1 = [30,30,30,30,30,   30,30,30,30,30,30,30, 30]
VI_level2 = [11,4,13,5,19,   29,21,29,30,9,13,9, 4]

directory1, directory2 = 'mud', 'mud'
...
def run_sim(label, ring, level, directory):
    ...
    return PF_cal_damage_burst(plot_graph, ring, level, time, print_option, directory)
def run_sim2(label, ring, level, directory):
    if compare == 1: print(f'{label} ---------------------')
    return PF_cal_damage_burst2(plot_graph, ring, level, time, print_option, directory)

res1 = run_sim('전', ring1, VI_1, directory1)
sum1, DC1, DCC1, time_int1 = res1

if compare == 1:
    res2 = run_sim2('후', ring2, VI_2, directory2)
