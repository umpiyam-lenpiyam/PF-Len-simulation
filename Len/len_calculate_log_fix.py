...
def len_cal_damage_fix(plot_graph, ring, VI_level, time, print_property, directory):
    start_time = tm.time()
    Boss_Guard = 380
    ...
    T20_2, T30_2 = [T2(stats, t) for t in (20, 30)]
    ...
    for t in range(0*1000, time*1000,iteration_time):
        ...
        # 어센트 3120
        낙화, 진천, 천강 = 0, 0, 0
        ascent_trigger_times = [45*1000, 45*1000+3120, 45*1000+6240]
        for trigger_s in ascent_trigger_times:
            elapsed_어센트 = t - trigger_s
            if 0 <= elapsed_어센트 <= 3120 and VI_level[9]:
                예인_lock = Skill_lock = 1
                pen_boss = 40 + 10 * min(2, VI_level[9] // 10)
                pen_ign  = 60 + 10 * min(2, VI_level[9] // 10) + (20 if VI_level[9] == 30 else 0)
                if 210 <= elapsed_어센트 <= 410 and (elapsed_어센트 - 210) % 50 == 0:
                    pct = (435 + 88 * VI_level[9]) * 7
                    낙화 = Ascent_D(pct, ascent_stats['Ignore_Guard'], pen_boss, pen_ign)
                    state["낙화"]["times_used"] += 1
                    state["낙화"]["attacks"] += 7
                if 1410 <= elapsed_어센트 <= 2250 and (elapsed_어센트 - 1410) % 60 == 0:
                    pct = (770 + 156 * VI_level[9]) * 10
                    진천 = Ascent_D(pct, ascent_stats['Ignore_Guard'], pen_boss, pen_ign)
                    state["진천"]["times_used"] += 1
                    state["진천"]["attacks"] += 10  
                if 2340 <= elapsed_어센트 <= 3060 and (elapsed_어센트 - 2340) % 30 == 0:
                    pct_낙화 = (435 + 88 * VI_level[9]) * 7
                    낙화 = Ascent_D(pct_낙화, ascent_stats['Ignore_Guard'], pen_boss, pen_ign)
                    state["낙화"]["times_used"] += 1
                    state["낙화"]["attacks"] += 7  
                    pct_천강 = (528 + 105 * VI_level[9]) * 15
                    천강 = Ascent_D(pct_천강, ascent_stats['Ignore_Guard'], pen_boss, pen_ign)
                    state["천강"]["times_used"] += 1
                    state["천강"]["attacks"] += 15
                break
...
#-------------------------------------------------------------------------------------------------------------------------------            
        ...
        if state["일격예인"]["delay"] <= 0 and not is_combo_active and 예인_lock == 0 and (state["일격예인"]["times_used"] > 0 or t >= Condition_ARRAY[8]):
            state["일격예인"]["delay"] = T30_2 - iteration_time
        ...
        if state["열지"]["action_delay"] <= 0:
            ...
            if can_burst or can_normal:
                ...
                if can_burst:
                    ...
                    if state["열지"]["burst_count"] == 3:
                        state["열지"]["delay"] = T20 - iteration_time - 2520
                ...
                state["열지"]["action_delay"] = (90 if 예인_lock == 1 else 510) - iteration_time
                ...
                state["열지"]["action_delay"] = (90 if 예인_lock == 1 else 630) - iteration_time
                ...
                state["열지"]["action_delay"] = (570 if 예인_lock == 1 else 510) - iteration_time
...
# 추가타 ------------------------------------------------------------------        
        ...
        if VI_level[7] > 0:
            ...
            if current_tick_uses > 0:
                ...
                if trigger_count > 0:
                    ...
                    망혼강림 = D(pct, VMatrix_Ignore_Guard, 0, 0, Final_Damage*Grandis_Final_Damage)
        ...
        # 화중군자 VI   
        화중군자 = 0
        if VI_level[9] == 0 or not (0 <= elapsed_어센트 <= 3120):
            if VI_level[11] > 0:
        ...
        if 만리향_active == 1:
            ...
            if immediate_chammae > 0 or added_count > 0:
                ...
                total_pct = 0
                total_pct += 280 * 3 * immediate_chammae * VI_level_factor[0]
                if blossoms > 0:
        ...
        if 망혼각성_active == 1:
            ...
            if immediate_cheona > 0 or added_count_v2 > 0:
                ...
                total_pct_v2 = 0
                total_pct_v2 += 600 * 2 * immediate_cheona * VI_level_factor[1]
                if cheongeop_triggered:
                    total_pct_v2 += 1950 * 12 * 9 * VI_level_factor[1]
        ...
        if VI_level[10] > 0 and hecate_lock == 0:
            if state["Hecate"]["delay"] <= 0:
                ...
                Hecate = D((330 + 23 * VI_level[10]) * 24, Ignore_Guard2, h_bonus, h_bonus, Final_Damage)
        ...
        CoS, SiM = 0, 0
        if elapsed_com == 0: 
            CoS = D(4621 * 12, Ignore_Guard2, 0, 0, Final_Damage)
            state["CoS"]["times_used"] += 1
            state["CoS"]["attacks"] += 12
            SiM = D(2773 * 15, Ignore_Guard2, 0, 0, Final_Damage)
            state["SiM"]["times_used"] += 1
            state["SiM"]["attacks"] += 15
        if 0 < elapsed_com <= 48300 and elapsed_com % 2100 == 0:
            CoS = D(1694 * 6, Ignore_Guard2, 0, 0, Final_Damage)
            state["CoS"]["times_used"] += 1
            state["CoS"]["attacks"] += 6
        if 0 < elapsed_com <= 49980 and elapsed_com % 8330 == 0: 
            SiM = D(1078 * 8 * 5, Ignore_Guard2, 0, 0, Final_Damage)
    ...
    Deal = np.vstack([state[name]["damage"] for name in skill_order]) * 4 * (1.3 * 1 * 1.2 * (50 + 5/2)/100 * 1.25 * (1 + 0.91)/2) / (10**12)
    ...
    write_skill_log(total_sum, Deal_Cumulative, skill_order, state, 'len_log.txt')
