...
#-------------------------------------------------------------------------------------------------------------------------------            
        ...
        # 일격예인
        일격예인, 일격예인_사용 = 0, 0
        elapsed_예인 = t % T30 - Condition_ARRAY[8] 
        if 0 <= elapsed_예인 < 540:
            Skill_lock = 1
        if 0 <= elapsed_예인 < 1750 and elapsed_예인 % 90 == 0:
            pct = 2.2 * (246 + 4 * VI_level[8]) * 7 if VI_level[8] > 0 else 2.2 * 214 * 7
...
# 추가타 ------------------------------------------------------------------        
        ...
        if 만리향_active == 1:
            ...
            if immediate_chammae > 0 or added_count > 0:
                ...
                if old_count < 150 and new_count >= 150: blossoms += 65
                total_pct = 0
